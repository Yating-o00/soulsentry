...
export default function ProductHeatmap({ tasks, notes, behaviors }) {
  const { grid, maxVal, peakInfo, daySummary } = useMemo(() => {
    ...
    tasks?.forEach(t => {
      ...
      [t.completed_at, t.reminder_time, t.created_date].filter(Boolean).forEach(d => {
    ...
    notes?.forEach(n => {
      if (n.deleted_at || !n.created_date) return;
    ...
    const typeLabels = {
      ...
      morning: { label: "上午高效", icon: Sun, desc: "9-12点是你的黄金时段，适合安排深度工作" },
      noon: { label: "午间活跃", icon: Sun, desc: "你习惯在中午处理事务，注意适当休息" },
      afternoon: { label: "下午专注", icon: Sunset, desc: "2-6点是你的专注时段，可安排复杂任务" },
      evening: { label: "夜间活跃", icon: Moon, desc: "晚间6-10点更高效，注意平衡作息" },
      night: { label: "夜猫子", icon: Moon, desc: "深夜工作偏好明显，建议关注睡眠质量" },
    };
  ...
  return (
    <div className="space-y-4">
      {peakInfo && (
        <div className="flex items-center gap-3 p-3.5 bg-gradient-to-r from-[#384877]/5 to-transparent rounded-xl border border-[#384877]/10">
          ...
          <Link to="/Dashboard" className="text-xs text-[#384877] hover:underline flex items-center gap-0.5">
      ...
      {daySummary.peak && (
        <div className="flex gap-2">
          <div className="flex-1 bg-emerald-50 rounded-xl p-3 border border-emerald-100">
            <div className="text-xs text-emerald-600 font-medium">最活跃</div>
            <div className="text-sm font-bold text-emerald-700 mt-0.5">{DAYS[parseInt(daySummary.peak[0])]}</div>
          </div>
          {daySummary.low && daySummary.low[0] !== daySummary.peak[0] && (
            <div className="flex-1 bg-amber-50 rounded-xl p-3 border border-amber-100">
              <div className="text-xs text-amber-600 font-medium">最空闲</div>
              <div className="text-sm font-bold text-amber-700 mt-0.5">{DAYS[parseInt(daySummary.low[0])]}</div>
            </div>
          )}
        </div>
      )}
            ...
            {DAYS.map((day, dayIdx) => (
              <div key={day} className="flex items-center gap-0.5 mb-0.5">
                ...
                {displayHours.map(hour => {
                  ...
                  return (
                    <div key={hour} className={`flex-1 aspect-square rounded-sm ${getHeatColor(val, maxVal)} transition-colors`} title={`${day} ${hour}:00 - ${val}次活动`} />
                  );
                })}
              </div>
            ))}
