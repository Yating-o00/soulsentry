...
function renderImgs(imgs, theme) {
...
function renderBullets(bs, cls = "") {
  return bs.length
    ? `<ul class="${cls}">${bs.map((b) => `<li><span class="dot"></span><span class="li-text">${escapeHtml(b)}</span></li>`).join("")}</ul>`
    : "";
}

function inferLayout(s, i, slides) {
...
export function renderPptHtml(data) {
  ...
  const slideHtml = slides.map((s, i) => {
    ...
    const imgs = Array.isArray(s.images)
      ? s.images
      : (s.image_url ? [{ url: s.image_url, caption: s.image_caption || "" }] : []);
    const pageNum = String(i + 1).padStart(2, "0");
    const totalNum = String(slides.length).padStart(2, "0");
    const layout = inferLayout(s, i, slides);
    ...
    if (layout === "section-divider") {
      return `<section class="slide section-divider"><div class="sd-inner"><div class="sd-num">${pageNum}</div><h1 class="sd-title">${heading}</h1>${body ? `<p class="sd-sub">${body}</p>` : ""}</div></section>`;
    }
    if (layout === "quote") {
      return `<section class="slide quote">${corner}${tBar}<blockquote class="quote-block"><span class="qmark">"</span><p>${body || (bullets[0] ? escapeHtml(bullets[0]) : "")}</p></blockquote></section>`;
    }
    if (layout === "stats") {
      const stats = (Array.isArray(s.stats) && s.stats.length)
        ? s.stats
        : bullets.slice(0, 4).map((b) => {
            const m = String(b).match(/^([^\s：:]+)\s*[：:]?\s*(.+)$/);
    ...
    if (layout === "image-full" && imgs.length) {
      return `<section class="slide image-full">${corner}<div class="if-bg" style="background-image:url('${escapeHtml(imgs[0].url)}')"></div><div class="if-overlay"></div><div class="if-text"><h2 class="if-h2">${heading}</h2>${imgs[0].caption ? `<p class="if-cap">${escapeHtml(imgs[0].caption)}</p>` : ""}${body ? `<p class="if-body">${body}</p>` : ""}</div></section>`;
    }
    if ((layout === "image-left" || layout === "image-right") && imgs.length) {
      const dir = layout === "image-right" ? "reverse" : "normal";
      return `<section class="slide has-img dir-${dir}">${corner}${tBar}<div class="content"><div class="imgs-col">${renderImgs(imgs, t)}</div><div class="text-col">${renderBullets(bullets)}${body ? `<p class="body">${body}</p>` : ""}</div></div></section>`;
    }
    if (layout === "cards") {
    ...
    if (layout === "two-column") {
      ...
      const colHtml = colB.length
        ? `<div class="two-col"><div>${renderBullets(colA, "big-bullets")}</div><div>${renderBullets(colB, "big-bullets")}</div></div>`
        : renderBullets(bullets, "big-bullets");
      return `<section class="slide">${corner}${tBar}${colHtml}${body ? `<p class="body">${body}</p>` : ""}</section>`;
    }
    if (layout === "closing") {
      return `<section class="slide closing"><div class="deco-circle a"></div><div class="deco-circle b"></div><div class="closing-inner"><div class="kicker">THANK YOU</div><h1>${heading || "谢谢观看"}</h1>${body ? `<p class="sub">${body}</p>` : ""}${bullets.length ? `<div class="meta-row">${bullets.map((b) => `<span class="chip">${escapeHtml(b)}</span>`).join("")}</div>` : ""}</div></section>`;
    }
...
export function savePptHtml({ data, execution, fileBaseName }) {
  const uploadRoot = path.join(process.cwd(), env.UPLOAD_DIR);
  fs.mkdirSync(uploadRoot, { recursive: true });
  const safeTitle = String(fileBaseName || data.title || "ppt")
    .replace(/[\\/:*?"<>|]/g, "_")
    .slice(0, 40);
  const suffix = execution?.id ? `_${execution.id.slice(-6)}` : "";
  const filename = `${Date.now()}_ppt_${safeTitle}${suffix}.html`;
  const filePath = path.join(uploadRoot, filename);
  fs.writeFileSync(filePath, renderPptHtml(data), "utf8");
