...
import { 
  ...
  XCircle, Zap, Clock, AlertTriangle, Sparkles,
  FileText, CalendarCheck, StickyNote
} from "lucide-react";
...
const statusConfig = {
  parsing: { label: "AI解析中", color: "bg-indigo-50 text-indigo-600 border-indigo-200", icon: Sparkles, animate: true },
...
export default function ExecutionItem({ execution, onRetry, onConfirm, onDismiss }) {
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-shrink-0 flex flex-col items-center gap-1.5">
            <div className={`w-11 h-11 rounded-xl border flex items-center justify-center text-xl ${
              execution.execution_status === "executing" || execution.execution_status === "parsing" 
            ...
            <div className="flex justify-between items-start mb-1">
              <div className="flex-1 min-w-0">
                ...
                {execution.original_input && (
                  <p className="text-xs text-slate-500 mt-0.5 line-clamp-1">{execution.original_input}</p>
                )}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                <span className="text-[11px] text-slate-400">
                  {format(new Date(execution.created_date), "MM-dd HH:mm", { locale: zhCN })}
                </span>
                <Badge variant="outline" className={`text-[10px] gap-1 ${status.color}`}>
                  <StatusIcon className={`w-3 h-3 ${status.animate ? "animate-pulse" : ""}`} />
                  {status.label}
                </Badge>
              </div>
            </div>
              ...
              <div className="mt-3 p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium text-slate-700">执行编排</span>
                  </div>
                  <span className="text-[11px] text-slate-400">{completedSteps}/{totalSteps}</span>
                </div>
                <ExecutionStepFlow steps={execution.execution_steps} />
              </div>
            )}

            {/* AI Parsed result summary */}
            {execution.ai_parsed_result?.summary && (
              <div className="mt-2 px-2.5 py-1.5 rounded-lg bg-indigo-50/50 border border-indigo-100">
                <div className="flex items-center gap-1.5">
                  <Sparkles className="w-3 h-3 text-indigo-500 flex-shrink-0" />
                  <span className="text-xs text-indigo-700">{execution.ai_parsed_result.summary}</span>
                </div>
              </div>
            )}
            ...
            {execution.execution_status === "waiting_confirm" && (
              <div className="mt-3 flex items-center justify-between p-2.5 rounded-lg bg-amber-50 border border-amber-200">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-amber-700">需要确认执行</span>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-slate-500" onClick={(e) => { e.stopPropagation(); onDismiss && onDismiss(execution); }}>
                    忽略
                  </Button>
                  <Button size="sm" className="h-7 text-xs bg-amber-500 hover:bg-amber-600 text-white" onClick={(e) => { e.stopPropagation(); onConfirm && onConfirm(execution); }}>
                    确认执行
                  </Button>
                </div>
              </div>
            ...
            {execution.execution_status === "failed" && (
              <div className="mt-3 flex items-center justify-between p-2.5 rounded-lg bg-red-50 border border-red-200">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-xs font-medium text-red-700">执行失败</span>
                    {execution.error_message && (
                      <p className="text-[11px] text-red-500">{execution.error_message}</p>
                    )}
                  </div>
                </div>
                <Button size="sm" variant="outline" className="h-7 text-xs border-red-200 text-red-600 hover:bg-red-100" onClick={(e) => { e.stopPropagation(); onRetry && onRetry(execution); }}>
              ...
              {execution.ai_parsed_result && (
                <div className="p-3 rounded-lg bg-indigo-50/50 border border-indigo-100">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {execution.ai_parsed_result.intent && (
                      <div><span className="text-slate-500">意图：</span><span className="text-slate-700">{execution.ai_parsed_result.intent}</span></div>
                    )}
                    {execution.ai_parsed_result.priority && (
                      <div><span className="text-slate-500">优先级：</span><span className="text-slate-700">{execution.ai_parsed_result.priority}</span></div>
                    )}
                    {execution.ai_parsed_result.time_expression && (
                      <div><span className="text-slate-500">时间：</span><span className="text-slate-700">{execution.ai_parsed_result.time_expression}</span></div>
                    )}
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-1.5">
                    {execution.execution_steps.map((step, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        {step.timestamp && (
                          <span className="text-slate-400 font-mono">{format(new Date(step.timestamp), "HH:mm")}</span>
                        )}
