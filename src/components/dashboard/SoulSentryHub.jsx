...
// Color mapping for inline styles or arbitrary tailwind classes
const colors = {
  void: '#0a0a0f',
  mist: '#f5f5f0',
  dawn: '#e8d5b7',
...
export default function SoulSentryHub({ initialData, initialShowResults = false }) {
  ...
  const DeviceCard = ({ id, device, active, onClick }) => {
    ...
    return (
        <div 
            onClick={() => onClick(id)}
            className={`
                device-card glass-refined rounded-2xl p-4 text-center cursor-pointer transition-all duration-300 border-2 
                ${active ? 'device-active' : 'border-transparent hover:border-[#e8d5b7]/30'}
                hover-lift
            `}
        >
            <div className={`
                w-10 h-10 mx-auto mb-3 rounded-2xl flex items-center justify-center text-[#f5f5f0] shadow-lg
                ${id === 'phone' ? 'bg-gradient-to-br from-[#0a0a0f] to-[#1e293b]' : ''}
                ${id === 'watch' ? 'bg-gradient-to-br from-[#1e293b] to-[#0a0a0f]' : ''}
            ...
            <h4 className="font-medium text-[#0a0a0f] text-sm mb-1">{device.name}</h4>
  ...
  return (
    <div className="soul-sentry-root w-full mx-auto relative overflow-hidden bg-[#f5f5f0] rounded-3xl min-h-[800px]">
        {/* Background Effects */}
        ...
        <div className="relative z-10 p-6 md:p-12 flex flex-col items-center">
                    ...
                    <div className="relative glass-refined rounded-3xl p-2">
                        <div className="bg-white/40 rounded-2xl flex flex-col">
                            <textarea
                                ...
                                className="w-full bg-transparent border-none outline-none text-lg text-[#0a0a0f] placeholder-[#0a0a0f]/30 resize-none px-6 py-5 font-light leading-relaxed scrollbar-hide min-h-[100px]"
                                disabled={isProcessing}
                            />
                            <div className="flex items-center justify-between px-4 pb-4">
                                <div className="flex gap-2">
                                    <button className="p-2 hover:bg-[#0a0a0f]/5 rounded-lg text-[#0a0a0f]/40 transition-colors">
                                ...
                                <button 
                                    ...
                                    className="btn-primary-soul px-6 py-2 rounded-full text-sm font-medium flex items-center gap-2 shadow-lg disabled:opacity-50"
                                >
                                    {isProcessing ? '分析中...' : '发送'}
                ...
                <div className="flex flex-wrap justify-center gap-3 mt-8">
                    {['今晚8点给妈妈打电话', '下周二完成Q4报告', '明早7点飞深圳'].map(text => (
                        <button 
                            ...
                            className="px-4 py-2 glass-refined rounded-full text-sm text-[#0a0a0f]/60 hover:text-[#0a0a0f] border border-transparent hover:border-[#e8d5b7]/50 transition-all"
                        >
                            {text}
                        </button>
                    ))}
            ...
            {showResults && (
                ...
                >
                    {/* Header with Reset */}
                    <div className="flex justify-between items-end border-b border-[#0a0a0f]/10 pb-4">
                        <div>
                            <h2 className="text-2xl font-serif text-[#0a0a0f]">已为你安排</h2>
                            <p className="text-sm text-[#0a0a0f]/50">基于输入: "{input}"</p>
                        </div>
                        <button 
                            onClick={() => { setShowResults(false); setInput(""); }}
                            className="text-sm text-[#0a0a0f]/60 hover:text-[#0a0a0f] underline"
                        >
                            新对话
                        </button>
                    </div>
                    ...
                    <section className="animate-fade-up">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-serif text-[#0a0a0f]">全设备智能协同</h3>
                            <span className="px-3 py-1 glass-refined rounded-full text-xs text-[#0a0a0f]/60 border border-[#e8d5b7]/20">
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        <h4 className="text-lg font-serif text-[#0a0a0f]">{data.devices[activeDevice].name} 策略</h4>
                                        <p className="text-sm text-[#0a0a0f]/50 mt-1">基于场景的智能分发</p>
                                    </div>
                                </div>
                                    ...
                                    {data.devices[activeDevice].strategies && data.devices[activeDevice].strategies.length > 0 ? (
                                        data.devices[activeDevice].strategies.map((strat, idx) => (
                                            <div key={idx} className="flex items-start gap-4 p-4 bg-white/40 rounded-xl border border-white/60 hover:bg-white/60 transition-colors">
                                                <div className="w-8 h-8 bg-[#e8d5b7]/20 rounded-full flex items-center justify-center text-[#0a0a0f]/70 text-sm font-serif flex-shrink-0">
                                                    {idx + 1}
                                                </div>
                                                <div className="flex-1">
                                                    <div className="flex justify-between items-start mb-1">
                                                        <span className="font-medium text-[#0a0a0f] text-sm">{strat.time}</span>
                                        ...
                                        <div className="text-center py-8 text-[#0a0a0f]/40 text-sm">该设备暂无特定策略</div>
                                    )}
                                </div>
                            </motion.div>
                        </AnimatePresence>
                    ...
                    <section className="animate-fade-up" style={{ animationDelay: '0.2s' }}>
                        <h3 className="text-xl font-serif text-[#0a0a0f] mb-6">情境感知时间线</h3>
                        <div className="glass-refined rounded-3xl p-8 relative">
                            {data.timeline.map((event, idx) => (
                                <div key={idx} className="timeline-item relative pl-12 pb-8 last:pb-0 group cursor-pointer">
                                    <div className="timeline-line"></div>
                                    <div className={`absolute left-0 top-0 w-12 h-12 rounded-full ${event.highlight ? 'bg-[#e8d5b7]/30' : 'bg-white/60'} flex items-center justify-center text-2xl border border-white/60 shadow-sm z-10 group-hover:scale-110 transition-transform`}>
                                        {event.icon}
                                    </div>
                                    <div className="pt-2">
                                        <div className="flex items-baseline gap-3 mb-1">
                                            <span className="font-mono text-[#0a0a0f]/40 text-sm">{event.time}</span>
                                            <h4 className={`font-serif text-lg ${event.highlight ? 'text-[#0a0a0f]' : 'text-[#0a0a0f]/80'}`}>{event.title}</h4>
                                            {event.highlight && <span className="px-2 py-0.5 bg-[#e8d5b7]/20 text-[#0a0a0f]/70 text-[10px] rounded-full">重点</span>}
                                        </div>
                                        <p className="text-[#0a0a0f]/50 text-sm leading-relaxed max-w-md">{event.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <section className="animate-fade-up" style={{ animationDelay: '0.3s' }}>
                        <div className="flex items-center gap-3 mb-6">
                            <h3 className="text-xl font-serif text-[#0a0a0f]">自动执行清单</h3>
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-1.5 h-1.5 rounded-full ${
                                                        ...
                                                        task.status === 'ready' ? 'bg-[#6366f1]' : 'bg-amber-400'
                                                    }`} />
                                        ...
                                        {task.status === 'ready' && (
                                            <button 
                                                ...
                                                className="px-3 py-1.5 bg-[#0a0a0f] text-[#f5f5f0] text-xs rounded-full hover:bg-[#1e293b] transition-colors shadow-md"
                                            >
                                                执行
                                            </button>
                                        )}
