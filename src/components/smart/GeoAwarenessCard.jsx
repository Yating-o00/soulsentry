    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 p-4 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-900 text-sm sm:text-base">地理情境感知</h3>
            <p className="text-xs text-slate-500 mt-0.5">{eventLabel} · 刚刚</p>
          </div>
        </div>
        <span className="self-start px-2.5 py-1 bg-blue-50 text-blue-600 text-xs rounded-full font-medium whitespace-nowrap">
          高优先级
        </span>
      </div>
      ...
      <div className="mx-4 mb-4 p-4 bg-blue-50/60 rounded-xl">
        <div className="flex items-start gap-2 mb-3">
          <Pin className="w-4 h-4 text-red-500 mt-0.5" />
        ...
        <ul className="space-y-2">
          {visibleTasks.map((t) => (
            <li key={t.id} className="flex items-start gap-2 text-xs sm:text-sm">
