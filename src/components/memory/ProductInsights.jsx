...
import {
  ...
  AlertTriangle, Flame, ArrowRight, Lightbulb, Activity
} from "lucide-react";
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {action && (
            <Link to={action.link} className="inline-flex items-center gap-1 text-xs text-[#384877] mt-1.5 hover:underline font-medium">
...
export default function ProductInsights({ tasks, notes, behaviors }) {
  const insights = useMemo(() => {
    ...
    // Completion rate
    const completionRate = last30Tasks.length > 0 ? Math.round((completedTasks.length / last30Tasks.length) * 100) : 0;
    ...
    return {
      ...
      topCategory: topCat ? catMap[topCat[0]] || topCat[0] : null,
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-3 md:grid-cols-2">
        <InsightCard
          icon={Target} iconColor="bg-emerald-100 text-emerald-600"
          title="30天执行效率" value={`${insights.completionRate}%`}
          desc={`近30天创建${insights.totalTasks}个约定，完成${Math.round(insights.totalTasks * insights.completionRate / 100)}个`}
          action={{ label: "查看约定", link: "/Tasks" }}
        />
        <InsightCard
          icon={Flame} iconColor="bg-orange-100 text-orange-600"
          title="连续打卡" value={insights.streak > 0 ? `${insights.streak}天` : "今日待开始"}
          desc={insights.streak >= 7 ? "连续一周高效执行，继续保持！" : "每日完成至少一个约定即可累计"}
        />
        <InsightCard
          icon={Clock} iconColor="bg-amber-100 text-amber-600"
          title="时间估算校准" value={insights.avgDelay > 0 ? `+${insights.avgDelay}分钟` : "准时"}
          desc={insights.avgDelay > 15 ? `约定平均延迟${insights.avgDelay}分钟，建议预留缓冲` : "时间把控准确，继续保持"}
          action={insights.avgDelay > 15 ? { label: "优化规划", link: "/Dashboard" } : undefined}
        />
        <InsightCard
          icon={Activity} iconColor="bg-purple-100 text-purple-600"
          title="状态预测" value={insights.peakDay || "数据积累中"}
          desc={insights.peakDay ? `${insights.peakDay}是高产日${insights.lowDay ? `，${insights.lowDay}效率偏低` : ""}` : "继续使用以学习你的能量节律"}
        />
        <InsightCard
          icon={BarChart3} iconColor="bg-blue-100 text-blue-600"
          title="重心分布" value={insights.topCategory || "均衡"}
          desc={insights.topCategory ? `${insights.topCategory}类占比最高(${insights.topCategoryCount}个)` : "各类约定分布均衡"}
        />
        <InsightCard
          icon={Brain} iconColor="bg-indigo-100 text-indigo-600"
          title="知识沉淀" value={`${insights.noteCount}篇心签`}
          desc={`近30天覆盖${insights.tagCount}个主题领域`}
          action={{ label: "查看心签", link: "/Notes" }}
        />
        {insights.overdueCount > 0 && (
          <InsightCard
            icon={AlertTriangle} iconColor="bg-red-100 text-red-600"
            title="风险预警" value={`${insights.overdueCount}个逾期`}
            desc="建议优先处理逾期约定或调整截止时间"
      ...
      <div className="bg-gradient-to-r from-[#384877]/5 via-white to-purple-50/30 rounded-2xl p-4 border border-slate-100">
        ...
        <div className="space-y-1.5 text-sm text-slate-600">
          ...
          {insights.peakDay && <p>• 高产日：<span className="font-semibold text-purple-600">{insights.peakDay}</span>，建议安排重要事项</p>}
          {insights.avgDelay > 0 && <p>• 时间校准：每项约定预留额外 <span className="font-semibold text-amber-600">{insights.avgDelay}分钟</span></p>}
          <p>• 知识沉淀 <span className="font-semibold text-indigo-600">{insights.noteCount}篇</span>，覆盖 <span className="font-semibold text-indigo-600">{insights.tagCount}个领域</span></p>
        </div>
      </div>
    </div>
  );
