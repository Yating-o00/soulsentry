...
import {
  ...
  Calendar, AlertTriangle, CheckCircle2, Flame, ArrowRight,
  Lightbulb, Timer, Activity
} from "lucide-react";
...
export default function ProductInsights({ tasks, notes, behaviors, executions }) {
  const insights = useMemo(() => {
    ...
    const last30Notes = activeNotes.filter(n => moment().diff(moment(n.created_date), "days") <= 30);

    // 1. Completion rate
    const completionRate = last30Tasks.length > 0
      ? Math.round((completedTasks.length / last30Tasks.length) * 100)
      : 0;

    // 2. Time estimation calibration
    const tasksWithTime = completedTasks.filter(t => t.reminder_time && t.completed_at);
    let avgDelay = 0;
    if (tasksWithTime.length > 3) {
      const delays = tasksWithTime.map(t => {
        return (new Date(t.completed_at) - new Date(t.reminder_time)) / (1000 * 60);
      }).filter(d => d > 0 && d < 1440);
      avgDelay = delays.length > 0 ? Math.round(delays.reduce((a, b) => a + b, 0) / delays.length) : 0;
    }

    // 3. Peak productivity day
    ...
    // 5. Streak: consecutive days with completed tasks
    let streak = 0;
    for (let i = 0; i < 30; i++) {
      const day = moment().subtract(i, "days").format("YYYY-MM-DD");
      const hasCompletion = completedTasks.some(t =>
        t.completed_at && moment(t.completed_at).format("YYYY-MM-DD") === day
      );
      if (hasCompletion) streak++;
      else break;
    }

    // 6. AI execution success rate
    ...
    const notesWithTags = last30Notes.filter(n => n.tags?.length > 0);
    ...
    return {
      ...
      streak, pendingCount: pendingTasks.length, overdueCount: overdueTasks.length,
      noteCount: last30Notes.length, tagCount: uniqueTags.size,
      execRate, execTotal: recentExec.length,
    ...
    <div className="space-y-3">
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <InsightCard
          ...
          title="连续打卡"
          value={insights.streak > 0 ? `${insights.streak}天` : "今日待开始"}
        ...
        <InsightCard
          ...
          title="时间估算校准"
          value={insights.avgDelay > 0 ? `+${insights.avgDelay}分钟` : "准时"}
          desc={insights.avgDelay > 15 ? `约定平均延迟${insights.avgDelay}分钟完成，系统建议自动添加缓冲时间` : "你的时间把控很准确，继续保持"}
        ...
        <InsightCard
          ...
          title="状态预测"
          value={insights.peakDay || "数据积累中"}
          desc={insights.peakDay
            ? `${insights.peakDay}是你的高产日${insights.lowDay ? `，${insights.lowDay}效率偏低` : ""}，可据此安排重要事项`
            : "继续使用，系统将学习你的能量节律"
          }
        />
        ...
        {insights.overdueCount > 0 && (
          <InsightCard
            ...
            title="风险预警"
            value={`${insights.overdueCount}个逾期`}
        ...
        {insights.execTotal > 0 && (
          <InsightCard
            ...
            title="AI辅助效率"
            value={`${insights.execRate}%`}
            desc={`近30天AI辅助执行${insights.execTotal}次，成功率${insights.execRate}%`}
            action={{ label: "查看执行", link: "/Notifications" }}
          />
        )}
      </div>
