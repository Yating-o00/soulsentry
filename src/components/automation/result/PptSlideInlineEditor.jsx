...
import { PPT_LAYOUTS } from "./pptLayouts";

// 简单的当前页内联编辑器：标题 / 副标题（仅封面）/ 要点列表 / 正文 / 版式
  ...
  return (
    <div className="w-full max-w-full text-white">
      ...
      <div className="flex items-center gap-1.5 mb-2">
        <LayoutIcon className="w-3 h-3 text-white/60 flex-shrink-0" />
        <span className="text-[10px] text-white/60 flex-shrink-0">版式</span>
        <select
          value={slide.layout || ""}
          onChange={(e) => update({ layout: e.target.value })}
          className="flex-1 bg-white/10 hover:bg-white/15 text-white text-[10.5px] rounded px-1.5 py-0.5 outline-none focus:ring-2 focus:ring-white/40 transition cursor-pointer"
          title="切换当前页的版式模板"
        >
          <option value="" className="text-slate-900">自动</option>
          {PPT_LAYOUTS.map((lay) => (
            <option key={lay.id} value={lay.id} className="text-slate-900">{lay.label}</option>
          ))}
        </select>
      </div>
      <input
