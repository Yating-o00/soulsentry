...
import {
  CheckCircle2, Clock, StickyNote, Bell, ListTodo, Brain,
  TrendingUp, Calendar, ChevronDown, ChevronUp, Zap,
...
export default function ProductTimeline({ tasks, notes, executions }) {
  ...
  // Build unified timeline from real product data
  const allEvents = useMemo(() => {
    ...
    tasks?.forEach(t => {
      ...
      // Created tasks (recent 30 days)
      if (t.created_date) {
        const daysDiff = moment().diff(moment(t.created_date), "days");
        if (daysDiff <= 30) {
          events.push({
            type: "task_created",
            date: t.created_date,
      ...
      // Overdue tasks
      if (t.status === "pending" && t.reminder_time && new Date(t.reminder_time) < new Date()) {
        events.push({
          type: "task_overdue",
          date: t.reminder_time,
          title: `⚠ ${t.title}`,
    ...
    notes?.forEach(n => {
      ...
      const daysDiff = moment().diff(moment(n.created_date), "days");
      if (daysDiff <= 60) {
    ...
    executions?.forEach(ex => {
      if (ex.execution_status === "completed" || ex.execution_status === "failed") {
        events.push({
          type: "execution",
          date: ex.completed_at || ex.created_date,
          title: ex.task_title,
          description: ex.ai_parsed_result?.summary,
          ...
          <>
            ...
            {allEvents.length > limit && (
              <button
                ...
                className="w-full py-3 text-sm text-[#384877] hover:bg-slate-50 rounded-xl transition-colors"
              >
                加载更多...
              </button>
            )}
