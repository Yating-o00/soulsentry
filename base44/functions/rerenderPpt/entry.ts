...
/**
 * 重新渲染 PPT —— 用户在前端切换"主题(theme)"或"单页版式(layout)"后调用。
 * 不重新调 AI、不扣费，只用现有 slides 数据按新参数渲染 HTML 上传。
 *
 * 入参：
 *   - execution_id: string  必填，指向已完成的 PPT TaskExecution
 *   - theme?: 'business' | 'minimal' | 'tech'    可选，要切换的整体风格
 *   - slides?: Array                              可选，前端改过 layout 的新 slides
 *
 * 出参：{ success, file_url, file_name, theme, slides }
 *
 * 注意：此函数把渲染逻辑独立拷贝过来（与 executeAutomation 中的 renderPptHtml 保持同步），
 * 不通过 import，因为 Deno backend 不允许跨文件 import。
 */
...
<body><div class="deck" id="deck">${slideHtml}</div>
...
Deno.serve(async (req) => {
  try {
    ...
    const { execution_id, theme, slides } = await req.json();
    ...
    const oldData = exec.automation_result?.data || {};
    if (!Array.isArray(oldData.slides) && !Array.isArray(slides)) {
      return Response.json({ error: 'No slides in execution; cannot rerender.' }, { status: 400 });
    }

    const nextData = {
      ...oldData,
      title: oldData.title || '演示稿',
      subtitle: oldData.subtitle || '',
      theme: theme || oldData.theme || 'business',
      slides: Array.isArray(slides) ? slides : oldData.slides,
    };

    // 渲染
    const html = renderPptHtml(nextData);

    // 上传新 HTML（覆盖式不可能，所以生成新文件名，下载链接会更新）
    const safeTitle = String(nextData.title || 'ppt').replace(/[\\/:*?"<>|]/g, '_').slice(0, 40);
    ...
    // 把新 data 写回 automation_result，前端拿到后会自动刷新
    const updatedResult = {
      ...(exec.automation_result || {}),
      type: 'ppt_doc',
      data: { ...nextData, file_name: fileName, file_url: fileUrl },
    };
    await base44.entities.TaskExecution.update(execution_id, { automation_result: updatedResult });
